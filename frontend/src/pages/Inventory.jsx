    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="max-w-xl mx-auto mb-8">
          ...
          <form onSubmit={handleSubmit} className="bg-white/90 dark:bg-gray-800/90 backdrop-blur rounded-2xl shadow-lg p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {[{ key: 'all', label: 'All' }, { key: 'soon', label: 'Expiring Soon' }, { key: 'expired', label: 'Expired' }].map((tab) => (
            <button key={tab.key} onClick={() => setFilter(tab.key)}
              className={`px-4 py-2 rounded-full text-sm font-semibold transition ${
                filter === tab.key ? 'bg-emerald-500 text-white shadow' : 'bg-white/90 dark:bg-gray-800/90 text-gray-600 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-gray-700'
              }`}>
              {tab.label}
            </button>
          ))}
